...
export default function Sidebar({ onLogout, user, isDarkMode, toggleDarkMode }: SidebarProps) {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'} transition-all duration-300 ease-in-out ${isExpanded ? 'w-64' : 'w-16'}`}>
      <div className="flex flex-col h-full">
        <button 
          onClick={toggleSidebar} 
          className={`p-4 ${isDarkMode ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-blue-600'}`}
        >
          <Menu className="h-6 w-6" />
        </button>
        
        <div className="flex-grow">
          <button className={`w-full p-4 ${isDarkMode ? 'text-gray-300 hover:text-white hover:bg-gray-800' : 'text-gray-600 hover:text-blue-600 hover:bg-gray-200'}`}>
        ...
        <div className="mb-4">
          <button 
            ...
            className={`w-full p-4 ${isDarkMode ? 'text-gray-300 hover:text-white hover:bg-gray-800' : 'text-gray-600 hover:text-blue-600 hover:bg-gray-200'}`}
          >
